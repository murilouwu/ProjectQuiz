<!DOCTYPE html>
<html>
	<head>
		<meta charset="utf-8">
		<meta name="viewport" content="width=device-width, initial-scale=1">
		<link href="https:\\cdn.jsdelivr.net\npm\bootstrap@5.1.3\dist\css\bootstrap.min.css" rel="stylesheet" integrity="sha384-1BmE4kWBq78iYhFldvKuhfTAU6auU8tT94WrHftjDbrCEXSU1oBoqyl2QvZ6jIW3" crossorigin="anonymous">
		<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-ka7Sk0Gln4gmtz2MlQnikT1wXgYsOg+OMhuP+IlRH9sENBO0LRn5q+8nbTov4+1p" crossorigin="anonymous"></script>
		<link rel="stylesheet" type="text/css" href="estilo/estile.css">
		<script src="java/fun.js"></script>
		<link rel="shortcut icon" href="img/play.png">
		<title>Play quiz</title>
	</head>
	<body>
		<div class="all">
			<nav class="navbar navbar-expand-lg navbar-dark bg-dark">
		        <div class="container-fluid d-flex justify-content-around">
			        <div class="d-flex">
			            <ul class="navbar-nav me-auto mb-2 mb-lg-0">
			                <li class="nav-item">
			                    <a class="nav-link text-warning linkcss" aria-current="page" href="index.html"><strong>♛</strong></a>
			                </li>
			                <li class="nav-item">
			                	<a class="nav-link text-warning linkcss" aria-current="page" href="pageUser.php"><strong>Perfil</strong></a>
			                </li>
			                <li class="nav-item">
			                	<a class="nav-link text-warning linkcss" aria-current="page" href="rank.php"><strong>Ranking</strong></a>
			                </li>
			        	</ul>
			        </div>
		        </div>
		    </nav>
		    <div class="position-relative overflow-hidden p-3 p-md-5 text-center bg-dark bg-opacity-50">
	            <div class="col-md-5 lg-5 mx-auto my-2">
	                <img src="img/logo.png" class="imglink">
	                <p class="lead fw-normal text-light">
	                    welcome to the quiz game
	                </p>
	            </div>
	        </div>
	        <div class="container d-flex flex-column justify-content-center">
	        	<div class="d-flex flex-column justify-content-center text-center align-items-center p-md-3">
	        		<h1 class="fw-normal text-light">Your quiz:</h1>
	        		<a class="nav-link text-warning bg-dark rounded-pill" style="width: 30%;" href="myquiz.php">To see</a>
	        		<h1 class="fw-normal text-light p-md-3">Quiz:</h1>
	        	</div>
	        	<div class="d-flex flex-row justify-content-center text-center align-items-center p-md-1">
	        		<div class="collumn bg-dark rounded-3 p-md-2" style="margin-right: 1vh;">
	        			<h2 class="text-light">name 1</h2>
	        			<a class="nav-link text-light linkcss" href="#"><strong>Play</strong></a>
	        			<h6 class="text-light">categoria: matematica</h6>
	        			<h6 class="text-light">peruntas: 10</h6>
	        		</div>
	        	</div>
	        </div>
		</div>
	</body>
</html>